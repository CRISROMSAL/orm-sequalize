...
export const getAll = async () => await sequelize.models.categorias.findAll();
export const getById = async (id) => await sequelize.models.categorias.findByPk(id);

export const create = async (data) => {
    try {
        console.log("🚀 [DEBUG] Iniciando proceso de creación...");
        const nuevo = await sequelize.models.categorias.create(data);
        ...
        if (LogTable) {
            await LogTable.create({ log: "Creación en categorias ID: " + nuevo.id });
...
export const update = async (id, data) => {
    const item = await sequelize.models.categorias.findByPk(id);
