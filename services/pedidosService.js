...
export const getAll = async () => await sequelize.models.pedidos.findAll();
export const getById = async (id) => await sequelize.models.pedidos.findByPk(id);

export const create = async (data) => {
    try {
        console.log("🚀 [DEBUG] Iniciando proceso de creación...");
        const nuevo = await sequelize.models.pedidos.create(data);
        ...
        if (LogTable) {
            await LogTable.create({ log: "Creación en pedidos ID: " + nuevo.id });
...
export const update = async (id, data) => {
    const item = await sequelize.models.pedidos.findByPk(id);
