...
export const getAll = async () => await sequelize.models.detalles_pedido.findAll();
export const getById = async (id) => await sequelize.models.detalles_pedido.findByPk(id);

export const create = async (data) => {
    try {
        console.log("🚀 [DEBUG] Iniciando proceso de creación...");
        const nuevo = await sequelize.models.detalles_pedido.create(data);
        ...
        if (LogTable) {
            await LogTable.create({ log: "Creación en detalles_pedido ID: " + nuevo.id });
...
export const update = async (id, data) => {
    const item = await sequelize.models.detalles_pedido.findByPk(id);
