...
export const getAll = async () => await sequelize.models.productos.findAll();
export const getById = async (id) => await sequelize.models.productos.findByPk(id);

export const create = async (data) => {
    try {
        console.log("🚀 [DEBUG] Iniciando proceso de creación...");
        const nuevo = await sequelize.models.productos.create(data);
        ...
        if (LogTable) {
            await LogTable.create({ log: "Creación en productos ID: " + nuevo.id });
...
export const update = async (id, data) => {
    const item = await sequelize.models.productos.findByPk(id);
