...
export const getAll = async () => await sequelize.models.log.findAll();
export const getById = async (id) => await sequelize.models.log.findByPk(id);

export const create = async (data) => {
    try {
        console.log("🚀 [DEBUG] Iniciando proceso de creación...");
        const nuevo = await sequelize.models.log.create(data);
        ...
        if (LogTable) {
            await LogTable.create({ log: "Creación en log ID: " + nuevo.id });
...
export const update = async (id, data) => {
    const item = await sequelize.models.log.findByPk(id);
